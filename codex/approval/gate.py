...
from codex.tools.base import BaseTool
from codex.types import ToolCall
...
class ApprovalGate:
    ...
    def check(self, tool: BaseTool, tool_call: Optional[ToolCall] = None) -> GateResult:
        ...
        # PLAN mode: all writes blocked
        if self.mode == Mode.PLAN:
            if tool.is_write or tool.is_shell:
                return GateResult(
                    decision=GateDecision.BLOCK,
                    reason=f"Plan mode: write/shell operations are blocked.",
                    tool_name=tool_name,
                )
            return GateResult(
        ...
        if self.policy == ApprovalPolicy.NEVER:
            if tool.is_write or tool.is_shell:
                return GateResult(
                    decision=GateDecision.BLOCK,
                    reason="Approval policy NEVER: all write operations blocked.",
                    tool_name=tool_name,
                )
            return GateResult(
                decision=GateDecision.ALLOW,
